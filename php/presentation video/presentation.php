...
<html lang="fr">
	<head>
		<meta charset="UTF-8">
		<meta name="keywords" content="HTML,CSS,PHP,videotheque,film,acteur,realisateur,action,comédie,presnetation,video">
		<meta name="description" content="page contenant toutes les informations d'un film">
		...
		<title>Page de présentation d'un film du site CINE ADDICT</title>
	</head>
<body>
	...
	<div class="container">
		<div class="image">
            <?php
            if (!empty($_GET['id'])){
                    $requetesql = $bdd->prepare('SELECT * FROM FILMS WHERE id = ?');
                    $requetesql->execute(array($_GET['id']));

                while ($donnee = $requetesql->fetch()) {
                    ?>
            <img src="../images/<?= $donnee['ID']?>.jpg" alt="Image de film" class="affiches" >
        </div>
        <div class="text">
            <h2 class="center">Titre : <?= $donnee['TITRE']?></h2>
            <p class="txt"><strong class="txtdes">Réalisateur :</strong> <a id="rea" href="../resultat/resultat.php?search=<?= $donnee['REALISATEUR']?>" title="lien vers films du réalisateur"><?= $donnee['REALISATEUR']?></a> </p>
            ...
            <p class="txt center"><strong class="txtdes">Description :</strong> <br>
                <?= $donnee['DESCRIPTION']?>
            <div class="prix center">
